...
- **Cuestionarios de evento (2026-06-11):** nuevo `EventFormController` en `/api/events/{id}/form` (GET ver, POST crear, DELETE borrar, POST `/vote`, POST `/response`). Un cuestionario por evento (`event_forms`, `event_form_votes`, `event_form_replies`); crear/borrar = profesional/admin (reglas en `SecurityConfig`, **antes** del `permitAll` de GET `/api/events/**`), ver/votar/responder = cualquier autenticado (anónimo incl.), 1 voto/respuesta por cuenta (restricción única `form_id`+`user_id`). Cada cambio se difunde por WebSocket en versión **pública** (sin `myVote`/`myResponded`) a `/topic/events/{id}/form` vía `WebSocketService.broadcastEventForm` (los textos de respuestas no se difunden, solo el contador).
...
- **Aviso de moderación en vivo:** los DOS caminos de "Avisar" — `ModerationService.warnMember` (lista de miembros) y `resolveReport` con acción `warn` (resolver un reporte) — incrementan `warnings` y **avisan al autor por WebSocket** vía `WebSocketService.sendUserNotification(username, "WARNING", warnings)` → `convertAndSendToUser(.., "/queue/notifications", ..)`. Solo lo recibe si está conectado (en vivo, no persistente). Ojo: en reportes de **historias** el autor es `"anonimo"` → no existe usuario, así que ahí no incrementa ni avisa. El DTO `ModerationMemberResponse` incluye `warnings` (para que el panel muestre el contador).
